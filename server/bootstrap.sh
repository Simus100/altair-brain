...
cp "$REPO/server/brain_core.py" "$API/brain_core.py"
cp "$REPO/server/update_brain.sh" "$API/update_brain.sh"
cp "$REPO/server/backup_data.sh" "$API/backup_data.sh"
chmod +x "$API/update_brain.sh" "$API/backup_data.sh"
mkdir -p "$DATA/inbox"
...
if [ ! -f "$API/.env" ]; then
  TOKEN="$(openssl rand -hex 32)"
  {
    ...
    echo "ALTAIR_INBOX_DIR=$DATA/inbox"
    echo "ALTAIR_UPDATE_SCRIPT=$API/update_brain.sh"
    echo "# Notifiche di guasto (gratuite): topic segreto su ntfy.sh, poi app ntfy sul telefono"
    echo "NTFY_TOPIC="
  } > "$API/.env"
  chmod 600 "$API/.env"
